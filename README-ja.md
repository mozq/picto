...
[ワイルドカード (glob)](https://en.wikipedia.org/wiki/Glob_%28programming%29) または [正規表現](https://ja.wikipedia.org/wiki/%E6%AD%A3%E8%A6%8F%E8%A1%A8%E7%8F%BE) でパターンを指定できます。
...
#### 操作
- コピー -- ファイルを対象フォルダーから保存先フォルダーにコピーします。
- 移動 -- ファイルを対象フォルダーから保存先フォルダーに移動します。
- 上書き -- 対象フォルダーのファイルを上書きします。
...
##### 変数値のフォーマット
'${FNumber:0.0}' のように ':' 区切り、または `format` フィルターを使うことで、変数値をフォーマットできます。
...
##### 変数値のマッチ
"${WhiteBalance?{0:Auto,1:Manual,default:''}}" のように match 式を使うことで、変数値に応じた文字列を返せます。

    ?{ <比較式> : <出力>, default : <出力> }

複数の比較式を指定すると、最初に一致した値が返されます。
どの比較式にも一致しない場合の値は `default` で指定できます。

    例)
    "${WhiteBalance?{0:Auto,1:Manual,default:''}}"
    -> "Auto" または "Manual" または "" (一致しない場合)
...
- 現在 -- 現在日時で変更します。
- 作成日時 -- ファイルの作成日時で変更します。
- 更新日時 -- ファイルの更新日時で変更します。
- アクセス日時 -- ファイルのアクセス日時で変更します。
- EXIF日時 -- ファイルのEXIF日時で変更します。
- 指定 -- 入力した日時で変更します。
...
#### テスト実行
実行ボタン横のメニューから、コピーやファイル変更を行わずに処理をシミュレーションできます。


## プリセット
現在の設定に名前を付けて保存し、**Presets**メニューから後で呼び出すことができます。

- **Save Current...**: 現在の設定をプリセットとして保存します。既存の名前を指定した場合は、上書きの確認が表示されます。
- メニュー内の保存済みプリセット名をクリックすると読み込まれます。
- **Manage Presets...**: 保存済みプリセットの名前変更・削除ができます。

プリセットには、対象/保存先フォルダーのパスを含む上記の設定内容がすべて含まれますが、言語や表示テーマは含まれません。
